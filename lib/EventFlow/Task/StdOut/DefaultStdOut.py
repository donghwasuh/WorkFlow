        ...
        while self.task_manager.shutdownFlag == False:
            if self.task_manager.status['status'] == 'ACT':
                try:
                    ...
                    if len(self.task_manager.stdout_deq_list) == 0:
                        self.task_manager.backup_deq.append(msg)
                        if self.debugMode: debugLog('stdout deq list is empty [%s] out -> backup_queue : %s' % ( self.task_manager.status['name'], repr(msg) ) )
