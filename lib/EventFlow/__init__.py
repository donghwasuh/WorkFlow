...
from EventFlow.Conf.ConfigDatabase      import ConfigDatabase       as ConfigDB
from EventFlow.Conf.NodeDatabase        import NodeDatabase         as NodeDB
from EventFlow.Conf.FlowDatabase        import FlowDatabase         as FlowDB
from EventFlow.Conf.ScheduleDatabase    import ScheduleDatabase     as ScheduleDB
from EventFlow.Conf.GroupDatabase       import GroupDatabase        as GroupDB

from EventFlow.Core                     import Core
from EventFlow.ConsoleListener          import ConsoleListener
from EventFlow.Scheduler                import Scheduler
from EventFlow.WebListener              import WebListener
from EventFlow.WebMonitor               import WebMonitor
from EventFlow.ResourceCollector        import ResourceCollector
from EventFlow.ResultCollector          import ResultCollector
...
# MainProcessor
# This class run in front of Event Flow
# Manage all daemon and kill them
class MainProcessor:
    def __init__(self, name, debugMode=True, host='127.0.0.1', port=9999):
        ...
        # load default env
        self.home_path = DefaultConfig.EVENTFLOW_HOME
        if not self.home_path:
            raise Exception, "EVENT FLOW HOME IS NOT SETTED!"
            sys.exit(1)
        ...
        #try:signal.signal(signal.SIGHUP, self.sig_handler)
        #except: normalLog("signal SIGHUP set failed.")
        #try: signal.signal(signal.SIGPIPE, self.sig_handler)
        #except: normalLog("signal SIGPIPE set failed.")
...
if __name__ == '__main__':
    #Log.Init()
    mp = MainProcessor("WCKANG_TEST", debugMode=True)
    mp.run()
